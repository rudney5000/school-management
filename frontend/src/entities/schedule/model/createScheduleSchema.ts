...
export const createScheduleSchema = z.object({
  ...
  startTime: z.string().regex(/^\d{2}:\d{2}$/, getErrorMessage('validation.timeFormat')),
  endTime: z.string().regex(/^\d{2}:\d{2}$/, getErrorMessage('validation.timeFormat')),
  room: z.string().max(50, getErrorMessage('validation.maxLength50')).optional().or(z.literal('')),
  academicYear: z
    .string()
  ...
  isLiveSession: z.boolean().optional(),
  liveUrl: z.string().url(getErrorMessage('validation.invalidUrl')).optional(),
});
