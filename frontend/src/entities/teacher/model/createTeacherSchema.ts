...
const teacherIdentitySchema = z.object({
  ...
  email: z.string().trim().email(getErrorMessage('validation.emailInvalid')),

  phone: z.string().max(20, getErrorMessage('validation.maxLength20')).optional().or(z.literal('')),
  ...
  dateOfBirth: z.string().min(1, getErrorMessage('validation.dateOfBirthRequired')),
});

const teacherAssignmentSchema = z.object({
  subSchoolId: z.string().uuid(),

  hireDate: z.string().min(1, getErrorMessage('validation.hireDateRequired')),
...
export const updateAssignmentSchema = teacherAssignmentSchema.partial().omit({ subSchoolId: true });
