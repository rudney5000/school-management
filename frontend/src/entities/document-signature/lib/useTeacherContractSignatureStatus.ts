...
import type { TeacherContractSignDto } from '@entities/document-signature/model/createDocumentSignatureSchema';

export const useTeacherContractSignatureStatus = (params: TeacherContractSignDto | undefined) => {
  const query = useQuery<SignatureStatusResult, Error>({
    queryKey: ['document-signature', 'teacher-contract', 'status', params],
    enabled: !!params?.subSchoolId && !!params?.teacherId,
    queryFn: async (): Promise<SignatureStatusResult> => {
      if (!params) return { isSigned: false };

      const response = await documentSignatureApi.getTeacherContractStatus(params);
