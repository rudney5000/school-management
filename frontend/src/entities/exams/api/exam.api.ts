...
export class ExamApi extends ApiWrapper {
  ...
  getAll(params?: ExamListQueryDto) {
    return this.handleRequest<Exam[]>(this._baseApi.get('/exams', params), (raw) => raw as Exam[]);
  }
  ...
  getMyChildrenExams(subSchoolId: string) {
    return this.handleRequest<Exam[]>(
      this._baseApi.get('/exams/me/children', { subSchoolId }),
      (raw) => raw as Exam[],
    );
  }

  create(payload: CreateExamDto) {
    return this.handleRequest<Exam>(this._baseApi.post('/exams', payload), (raw) => raw as Exam);
  }
  ...
  delete(id: string, subSchoolId: string) {
    return this.handleRequest(this._baseApi.delete(`/exams/${id}`, { subSchoolId }), undefined);
  }
