...
export const selectTypingUsers = (state: RootState): Record<string, string[]> =>
...
export const selectUnreadCount =
  ...
  (state: RootState): number =>
    (state.chat.messages[conversationId] ?? []).filter((m) => !m.isDeleted && m.senderId !== userId)
      .length;

export const selectUnreadCountByConversation =
  (userId: string) =>
  (state: RootState): Record<string, number> => {
    ...
    conversations.forEach((conv) => {
      ...
      result[conv.id] = convMessages.filter(
        (m) =>
          m.senderId !== userId &&
          !m.isDeleted &&
