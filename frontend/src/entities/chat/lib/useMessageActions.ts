import { chatActions, chatApi, type Message, useChatActions, useSocket } from '@entities/chat';
...
export function useMessageActions(conversationId: string) {
  ...
  const { starMessage, unstarMessage, archiveMessage, forwardMessage } = useChatActions(socketRef);
