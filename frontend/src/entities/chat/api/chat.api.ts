...
import type { Conversation, Message } from '@entities/chat/model/types';
import type {
  AddMembersInput,
  AddReactionInput,
...
export class ChatApi extends ApiWrapper {
  ...
  removeMember(id: string, userId: string) {
    return this.handleRequest(this._baseApi.delete(`/chats/${id}/members/${userId}`), undefined);
  }
  ...
  removeReaction(conversationId: string, messageId: string, emoji: string) {
    return this.handleRequest(
      this._baseApi.delete(
        `/chats/${conversationId}/messages/${messageId}/reactions/${encodeURIComponent(emoji)}`,
      ),
      undefined,
    );
  }
