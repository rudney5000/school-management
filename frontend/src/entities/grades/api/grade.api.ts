...
export class GradeApi extends ApiWrapper {
  ...
  getBulletin(params: BulletinQueryDto) {
    return this.handleRequest<any>(this._baseApi.get('/grades/bulletin', params), (raw) => raw);
  }

  create(payload: CreateGradeDto) {
    return this.handleRequest<Grade>(this._baseApi.post('/grades', payload), (raw) => raw as Grade);
  }
  ...
  delete(id: string) {
    return this.handleRequest(this._baseApi.delete(`/grades/${id}`), undefined);
  }
}
