...
import type { StudentParamsDto, StudentListQueryDto } from '@entities/student/model/dto';
import type { CreateStudentDto, Student, UpdateStudentDto } from '@entities/student';
...
export class StudentApi extends ApiWrapper {
  ...
  getUnassigned(subSchoolId: string) {
    return this.handleRequest<Student[]>(
      this._baseApi.get('/students/unassigned', { subSchoolId }),
      (raw) => raw as Student[],
    );
  }
  ...
  delete(id: string, subSchoolId: string) {
    return this.handleRequest(this._baseApi.delete(`/students/${id}`, { subSchoolId }), undefined);
  }
}
