...
import type { CreateSessionDto, JoinVideoCallResult, VideoCallSession } from '@entities/video-call';
...
export class VideoCallApi extends ApiWrapper {
  ...
  leave(sessionId: string) {
    return this.handleRequest(this._baseApi.post(`/video-calls/${sessionId}/leave`), undefined);
  }

  end(sessionId: string) {
    return this.handleRequest(this._baseApi.post(`/video-calls/${sessionId}/end`), undefined);
  }
}
