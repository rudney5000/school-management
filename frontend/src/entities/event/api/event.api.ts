...
import type { ParentParamsDto, ParentListQueryDto } from '@entities/parent/model/dto';
import type { CreateEventDto, Event, UpdateEventDto } from '@entities/event';
...
export class EventApi extends ApiWrapper {
  ...
  create(payload: CreateEventDto) {
    return this.handleRequest<Event>(this._baseApi.post('/events', payload), (raw) => raw as Event);
  }
  ...
  delete(id: string, subSchoolId: string) {
    return this.handleRequest(this._baseApi.delete(`/events/${id}`, { subSchoolId }), undefined);
  }
}
