  ...
  return (
    <div>
      <ColumnActions row={row}>
        {children}
        {onEditAction && (
          <DropdownMenuItem className="space-x-4" onClick={() => onEditAction(row.original)}>
            <Icons.pencil className="h-4 w-4" />
            <p>{t('dashboard.common.actions.edit')}</p>
          </DropdownMenuItem>
        )}
        {onDeleteAction && (
          <DropdownMenuItem className="space-x-4" onClick={() => onDeleteAction(row.original)}>
            <Icons.trash2 className="h-4 w-4" />
            <p>{t('dashboard.common.actions.delete')}</p>
          </DropdownMenuItem>
        )}
        {onViewAction && (
          <DropdownMenuItem className="space-x-4" onClick={() => onViewAction(row.original)}>
