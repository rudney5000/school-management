...
export function getClassColumns({ t, onEdit, onDelete }: ClassColumnsOptions): ColumnDef<Class>[] {
  return [
    {
      accessorKey: 'name',
      header: ({ column }) => (
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-muted text-muted-foreground text-[11px] font-medium flex-shrink-0">
            {(row.getValue('teacher') as string)
              ?.split(' ')
              .map((n) => n[0])
              .join('')
              .slice(0, 2)}
          ...
          <div className="flex flex-col gap-1.5 w-32">
            ...
            <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
              <div
                className={cn(
                  'h-full rounded-full',
                  ratio >= 100 ? 'bg-rose-500' : ratio >= 80 ? 'bg-amber-500' : 'bg-primary',
                )}
        ...
        >
          <span
            className={cn(
              ...
              row.getValue('isActive') ? 'bg-emerald-500' : 'bg-zinc-400',
            )}
          />
