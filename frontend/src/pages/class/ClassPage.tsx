...
import { ClassDetailPanel, EmptyDetailPanel } from '@/pages/class/ui';
...
const ClassesPage = () => {
  ...
  const stats = [
    ...
    {
      label: 'Enseignants',
      value: new Set(classes.map((c) => c.teacher).filter(Boolean)).size,
      icon: Presentation,
    ...
    },
    {
      ...
      value: classes.reduce((acc, c) => acc + c.capacity, 0),
      icon: Building2,
    ...
    <>
      <ClassesTable
        ...
        renderDetailPanel={(selected) =>
          selected ? <ClassDetailPanel classItem={selected} /> : <EmptyDetailPanel />
        }
        isLoading={isLoading}
      >
