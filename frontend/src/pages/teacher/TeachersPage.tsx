...
import { Search, UserCheck, UserPlus, Users, UserX, MoreVertical, Filter } from 'lucide-react';
import { Avatar, AvatarFallback, AvatarImage, Button, Input, Spinner } from '@shared/ui';
import { type StatCardItem } from '@shared/ui/data-table';
...
const TeachersPage = () => {
  ...
  const relatedTeachers = React.useMemo(() => {
    ...
    return (data ?? [])
      .filter(
        (t) => t.id !== selectedTeacher.id && t.specialization === selectedTeacher.specialization,
      )
      .slice(0, 5);
    ...
    <div className="flex h-screen bg-zinc-50">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {stat.trend && (
                      <span
                        className={cn(
                          'text-xs font-medium',
                          stat.trend.positive ? 'text-emerald-600' : 'text-rose-600',
                        )}
                      >
                        {stat.trend.value}
                      </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-zinc-600">{teacher.specialization || '—'}</span>
      ...
      {teacherToDelete && (
        <DeleteTeacherAlert
          ...
          onOpenChange={(open) => {
            if (!open) setTeacherToDelete(undefined);
          }}
