...
import type { AttendanceStatus, StudentAttendance } from '@entities/attendances';
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                {t('dashboard.widgets.classroomPulse.loading')}
              </p>
            </div>
          </div>
        </CardHeader>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              {pulseData.courseName} · {pulseData.className} · {pulseData.students.length}{' '}
              {t('dashboard.widgets.classroomPulse.students')}
            </p>
          </div>
        ...
        <div className="flex flex-wrap gap-3 text-xs text-zinc-500 mb-4">
          {(
            [
              {
                key: 'PRESENT',
                label: `${t('dashboard.widgets.classroomPulse.status.present')} (${pulseData.statusCounts.PRESENT})`,
              },
              {
                key: 'LATE',
                label: `${t('dashboard.widgets.classroomPulse.status.late')} (${pulseData.statusCounts.LATE})`,
              },
              {
                key: 'EXCUSED',
                label: `${t('dashboard.widgets.classroomPulse.status.excused')} (${pulseData.statusCounts.EXCUSED})`,
              },
              {
                key: 'ABSENT',
                label: `${t('dashboard.widgets.classroomPulse.status.absent')} (${pulseData.statusCounts.ABSENT})`,
              },
              {
                key: 'UNRECORDED',
                label: `${t('dashboard.widgets.classroomPulse.status.notRecorded')} (${pulseData.statusCounts.UNRECORDED})`,
              },
