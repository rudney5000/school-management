          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                {t('dashboard.widgets.gradeDistribution.loading')}
              </p>
            </div>
          </div>
        </CardHeader>
    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              {selectedCourseId === ALL_COURSES_ID
                ? t('dashboard.widgets.gradeDistribution.allCourses')
                : courses.find((c) => c.id === selectedCourseId)?.name}{' '}
              · {t('dashboard.widgets.gradeDistribution.currentPeriod')}
            </p>
          </div>
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={gradeData.distribution} barSize={48}>
        ...
        <div className="grid grid-cols-5 text-center mt-2 pt-3 border-t border-zinc-100 text-xs">
          {gradeData.distribution.map((grade) => (
            <div key={grade.grade}>
              <div className="font-bold text-zinc-800">{grade.students}</div>
        ...
        <div className="grid grid-cols-4 gap-3 mt-4 pt-4 border-t border-zinc-100">
          {[
            ...
            <div key={item.lbl} className="text-center">
              <div className={cn('text-base font-bold', item.color)}>{item.val}</div>

              <div className="text-[10px] text-zinc-400 leading-tight">{item.lbl}</div>
            </div>
          ))}
        </div>
      </CardContent>
