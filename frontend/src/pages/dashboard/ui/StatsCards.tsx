  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const [classesResponse, gradesResponse, attendanceResponse, studentsResponse] =
          await Promise.all([
        ...
        const classes =
          classesResponse.status === Status.Success && Array.isArray(classesResponse.result)
            ? classesResponse.result
            : [];

        const grades: Grade[] =
          gradesResponse.status === Status.Success && Array.isArray(gradesResponse.result)
        ...
        const students =
          studentsResponse.status === Status.Success && Array.isArray(studentsResponse.result)
        ...
        const attendanceRate =
          attendances.length > 0 ? ((presentCount / attendances.length) * 100).toFixed(1) : '0';

        const attendanceRecordedRate =
          totalStudents > 0 ? ((attendances.length / totalStudents) * 100).toFixed(1) : '0';
