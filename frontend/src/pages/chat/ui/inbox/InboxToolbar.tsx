...
export function InboxToolbar({ conversation, selectedMessage }: InboxToolbarProps) {
  ...
  const { starMessage, archiveMessage, deleteMessage, loadingId } = useMessageActions(
    conversation.id,
  );
    ...
    <div className="flex items-center justify-between border-b border-border px-4 py-2">
      ...
      <div className="flex items-center gap-1">
        <Button
          variant="ghost"
          size="icon-sm"
          className={cn('text-muted-foreground', selectedMessage?.isStarred && 'text-yellow-400')}
