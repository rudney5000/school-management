...
export function ConversationItem({
  ...
  const initials =
    displayName !== t('dashboard.chat.unnamedChat') &&
    displayName !== t('dashboard.chat.directMessage')
      ? displayName
          .split(' ')
          .map((n) => n[0])
          .join('')
  ...
  return (
    ...
    >
      ...
      {unread > 0 && (
        <Badge className="ml-auto shrink-0 rounded-full text-[10px] px-1.5 py-0.5">{unread}</Badge>
      )}
    </button>
  );
}
