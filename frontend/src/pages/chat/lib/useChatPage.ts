...
import { selectRole, selectUserId } from '@features/auth/model/selectors';
...
export function useChatPage() {
  ...
  const counts = useMemo(
    () => ({
      ...
      groups: conversations.filter(
        (c) => c.type === 'group' || c.type === 'class' || c.type === 'course',
      ).length,
      direct: conversations.filter((c) => c.type === 'dm').length,
      announcements: conversations.filter(
        (c) => c.type === 'announcement' || c.type === 'parent_group',
      ).length,
  ...
  useEffect(() => {
    if (conversations.length > 0 && !activeConversationId) {
      const first =
        conversations.find((c) => c.type === 'course' || c.type === 'class' || c.type === 'dm') ??
        conversations[0];
