...
import { getStatusBadgeClass, getTypeBadgeClass, type Exam } from '@entities/exams';
  ...
  const baseColumns: ColumnDef<Exam>[] = [
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          ...
          <span className="text-xs text-muted-foreground mt-0.5">
            {row.original.durationMinutes} min · /{row.original.maxScore} · coeff.{' '}
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getTypeBadgeClass(type)}`}
          >
