...
function gradeLevel(score: number) {
  if (score >= 16)
    return { label: 'Excellent', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
  ...
  if (score >= 10)
    return { label: 'Assez bien', color: 'bg-amber-50 text-amber-700 border-amber-200' };
    ...
    return exams
      .map((exam) => {
        const examGrades = grades.filter(
          (g) => g.courseId === exam.courseId && g.classId === exam.classId,
        );
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        >
          <SelectTrigger className="w-[200px]">
            <SelectValue placeholder={t('dashboard.exams.statistics.mode')} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="none">{t('dashboard.exams.statistics.none')}</SelectItem>
            <SelectItem value="class">{t('dashboard.exams.statistics.byClass')}</SelectItem>
            <SelectItem value="period">{t('dashboard.exams.statistics.byPeriod')}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>{t('dashboard.exams.statistics.distribution')}</CardTitle>
            <CardDescription>
              ...
              <div key={item.range} className="space-y-1.5">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">
                    {item.count}
                    {t('dashboard.exams.statistics.student')}·{item.percentage.toFixed(1)}%
                  </span>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('dashboard.exams.statistics.performanceLevels')}</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-2.5">
            {[
              ...
              return (
                <div
                  key={level.label}
                  className="flex items-center justify-between rounded-xl border border-border/60 px-4 py-2.5 transition-colors hover:bg-muted/40"
                >
      ...
      {comparisonMode !== 'none' && examComparison.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>{t('dashboard.exams.statistics.examComparison')}</CardTitle>
            <CardDescription>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('dashboard.exams.statistics.analysis')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
