...
import { Save, Check, X, AlertCircle, RefreshCw } from 'lucide-react';
...
export function GradeEntryGrid({ examId, onClose }: GradeEntryGridProps) {
  ...
  const handleScoreChange = (studentId: string, value: string) => {
    setRows((prev) =>
      prev.map((r) => (r.studentId === studentId ? { ...r, score: value, isDirty: true } : r)),
    );
  ...
  const handleCommentChange = (studentId: string, value: string) => {
    setRows((prev) =>
      prev.map((r) => (r.studentId === studentId ? { ...r, comment: value, isDirty: true } : r)),
    );
    setHasUnsavedChanges(true);
  };
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-muted-foreground text-center">
          <Spinner />
          <p className="mt-2 text-sm">{t('dashboard.exams.gradeEntry.loading')}</p>
        </div>
      </div>
    );
  }
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {isRetake && (
                  <Badge
                    variant="outline"
                    className="gap-1 text-amber-600 border-amber-600 shrink-0"
                  >
              ...
              <CardDescription className="mt-2">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs sm:text-sm">
                  ...
                  <span>
                    {t('dashboard.exams.gradeEntry.maxScore')}: {exam.maxScore}
                  </span>
                  ...
                  <div className="min-w-0">
                    ...
                    {row.result?.gradedAt && (
                      <div className="text-xs text-muted-foreground">
                        {t('dashboard.exams.gradeEntry.gradedOn')}{' '}
                        {new Date(row.result.gradedAt).toLocaleDateString()}
                      </div>
                    )}
                  </div>
                  <div className="md:hidden">{getScoreBadge(row.score, exam.maxScore)}</div>
                </div>
