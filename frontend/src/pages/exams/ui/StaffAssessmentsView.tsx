...
import { BarChart3, CalendarDays, ClipboardList, GraduationCap, PencilLine } from 'lucide-react';
import { useTranslation } from '@shared/lib';
import { Button, Tabs, TabsContent } from '@/shared/ui';
import { GradeEntryGrid } from '@/pages/exams/ui/GradeEntryGrid';
import { ResultsBulletin } from '@/pages/exams/ui/results-bulletin/ResultsBulletin';
import { StatisticsDashboard } from '@/pages/exams/ui/StatisticsDashboard';
import { type Exam, ExamStatus, useExams } from '@entities/exams';
...
import { AddExamForm, DeleteExamAlert, EditExamForm } from '@features/exams';
...
export function StaffAssessmentsView() {
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{t('dashboard.exams.subtitle')}</p>
            </div>
          </div>
        </div>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col space-y-6">
          ...
          <TabsContent value="exams" className="space-y-6">
            ...
            <EditExamForm
              ...
              isOpen={!!examToEdit}
              handleOpen={(open) => {
                if (!open) setExamToEdit(undefined);
              }}
            ...
            {examToDelete && (
              <DeleteExamAlert
                ...
                isOpen={!!examToDelete}
                onOpenChange={(open) => {
                  if (!open) setExamToDelete(undefined);
                }}
          ...
          <TabsContent value="grade-entry">
            {examToGrade ? (
              <GradeEntryGrid examId={examToGrade.id} onClose={() => setExamToGrade(undefined)} />
            ) : (
