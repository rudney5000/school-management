...
import { AddParentForm, ParentOverviewPanel } from '@features/parent';
...
const ParentsPage = () => {
  ...
  const stats = React.useMemo<StatCardItem[]>(() => {
    ...
    return [
      {
        label: t('dashboard.parents.stats.total'),
        value: total,
        icon: Users,
        trend: { value: `${total}`, positive: true },
      },
      {
        label: t('dashboard.parents.stats.active'),
        value: active,
        icon: UserCheck,
        trend: { value: `${total > 0 ? Math.round((active / total) * 100) : 0}%`, positive: true },
      },
      {
        label: t('dashboard.parents.stats.inactive'),
        value: inactive,
        icon: UserX,
        trend: {
  ...
  return (
    <ParentsTable
      ...
      renderDetailPanel={(parent) => (
        <ParentOverviewPanel parent={parent} onView={handleViewParent} onEdit={setParentToUpdate} />
      )}
    >
      <AddParentForm
        isOpen={isCreateOpen}
