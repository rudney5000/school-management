...
import type { Course, CourseColor, CourseIcon } from '@entities/courses';
    ...
    <Card className="flex flex-col hover:shadow-md transition-shadow duration-200">
          ...
          {(onEdit || onDelete) && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-36">
                <DropdownMenuLabel>{t('dashboard.courses.actions.title')}</DropdownMenuLabel>
                <DropdownMenuSeparator />
                {onEdit && (
      ...
      <CardFooter className="px-5 py-3">
        {course.status === 'completed' ? (
          ...
          <Badge variant="muted">{t('dashboard.courses.status.archived')}</Badge>
        ) : (
