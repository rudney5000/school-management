...
import { Badge, Button, Input, Separator, Spinner } from '@/shared/ui';
import { type Course, type CourseStatus, useCoursesList } from '@entities/courses';
...
export default function CoursesPage() {
  ...
  const STATUS_TABS = useMemo<{ key: CourseStatus | 'all'; label: string }[]>(
    () => [
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-3">
          ...
          {!isParent && (
            <Button size="sm" onClick={handleOpenCreate} className="gap-1.5 shrink-0">
              <Plus size={15} />
              <span className="hidden sm:inline">{t('dashboard.courses.add')}</span>
            </Button>
          )}
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 sm:py-24 text-muted-foreground">
            ...
            <p className="font-medium">{t('dashboard.courses.empty.title')}</p>
            <p className="text-sm mt-1">{t('dashboard.courses.empty.description')}</p>
          </div>
        ) : viewMode === 'grid' ? (
            ...
            {filtered.map((course) => (
              <CourseCard
                ...
                onDelete={
                  isParent
                    ? undefined
                    : (id) => setDeleteCourse(courses.find((c) => c.id === id) ?? null)
                }
              />
            ))}
          </div>
