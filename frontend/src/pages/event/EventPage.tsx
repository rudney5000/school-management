...
import { format, startOfWeek, addWeeks, addDays, isBefore } from 'date-fns';
import {
  TYPE_CONFIG,
  useEvents,
  useCreateEvent,
...
import { AvatarStack, buildGanttItems, WEEK_COUNT } from '@/pages/event/ui';
import {
  Plus,
  Clock,
  ChevronRight,
...
export default function EventsPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    await deleteEventMutation.mutateAsync({
      id: deletingEventId,
      subSchoolId: selectedSubSchoolId,
    });
    ...
    <div className="flex h-screen overflow-hidden bg-muted/30">
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="relative overflow-hidden rounded-2xl bg-sky-50 border border-sky-100 p-5 flex items-center gap-5 min-h-[100px]">
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                  {!loading &&
                    [...new Set(events.map((e) => e.type))].slice(0, 3).map((t) => (
                      <Badge
                        key={t}
              ...
              <div className="absolute right-4 bottom-0 text-6xl select-none opacity-25">🏫</div>
            </div>
            ...
            >
              ...
              <div className="absolute right-6 bottom-2 text-5xl select-none opacity-20">🌟</div>
            </div>
          </div>

          <div className="bg-background rounded-2xl border p-5 mb-6 overflow-x-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <h2 className="font-semibold text-base" style={{ color: 'oklch(0.45 0.18 280)' }}>
                      ...
                      <div key={rowIdx} className="relative h-11 mb-1.5">
                        {rowEvts.map((it) => {
                          ...
                          return (
                            ...
                            >
                              <span className="flex-1 min-w-0 truncate">{it.event.title}</span>
          ...
          <div>
            ...
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground">{t('dashboard.events.noPastEvents')}.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {pastEvents.map((ev) => {
                  ...
                  return (
                    <div key={ev.id} className="bg-background rounded-xl border p-4">
                      ...
                      <p className="font-semibold text-sm mb-1 line-clamp-1">{ev.title}</p>
                      <Badge className={cn('text-[10px] px-1.5 mb-2', cfg.badgeClasses)}>
                        {cfg.label}
                      </Badge>
      ...
      {!isParent && (
        <DeleteAlertEvent
          ...
          eventName={
            deletingEventId ? events.find((e) => e.id === deletingEventId)?.title : undefined
          }
        />
      )}
    </div>
