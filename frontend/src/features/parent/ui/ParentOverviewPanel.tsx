...
export function ParentOverviewPanel({ parent, onView, onEdit }: ParentOverviewPanelProps) {
  ...
  const quickActions: OverviewQuickAction[] = [
    ...(onView
      ? [{ icon: Eye, label: t('dashboard.common.actions.view'), onClick: () => onView(parent!) }]
      : []),
    ...(onEdit
      ? [
          {
    ...
    ...(parent?.phone
      ? [{ icon: Phone, label: parent.phone, href: `tel:${parent.phone}`, colSpan: 3 }]
      : []),
  ];
        ...
        details={
          parent
            ? [
                ...
                {
                  label: t('dashboard.parents.columns.gender'),
                  value:
                    parent.gender === 'male'
                ...
                {
                  label: t('dashboard.parents.columns.dateOfBirth'),
                  value: new Date(parent.dateOfBirth).toLocaleDateString('fr-FR'),
                },
              ]
            : []
        }
              ...
              return (
                ...
                >
                  <Avatar className="h-8 w-8">
                    <AvatarImage src={`https://i.pravatar.cc/150?u=${child.id}`} alt={fullName} />
