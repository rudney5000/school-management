import { GraduationCap, Mail, Pencil, Phone, Eye, Users } from 'lucide-react';
import { useTranslation, getInitials } from '@shared/lib';
import {
  EntityOverviewPanel,
...
export function TeacherOverviewPanel({
  teacher,
  onView,
  onEdit,
  relatedTeachers,
  ...
  const quickActions: OverviewQuickAction[] = [
    ...(onView
      ? [{ icon: Eye, label: t('dashboard.common.actions.view'), onClick: () => onView(teacher!) }]
      : []),
    ...(onEdit
      ? [
          {
            icon: Pencil,
            label: t('dashboard.common.actions.edit'),
    ...
    ...(teacher?.phone
      ? [{ icon: Phone, label: teacher.phone, href: `tel:${teacher.phone}`, colSpan: 3 }]
      : []),
  ];
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
        ...
        details={
          teacher
            ? [
                ...
                {
                  label: t('dashboard.teachers.columns.gender'),
                  value:
                    teacher.gender === 'male'
                ...
                {
                  label: t('dashboard.teachers.columns.dateOfBirth'),
                  value: new Date(teacher.dateOfBirth).toLocaleDateString('fr-FR'),
                },
                {
                  label: t('dashboard.teachers.columns.hireDate'),
                  value: new Date(teacher.hireDate).toLocaleDateString('fr-FR'),
                },
              ]
            : []
        }
      ...
      {teacher && <TeacherDossierCard teacherId={teacher.id} subSchoolId={teacher.subSchoolId} />}
        ...
        <div className="rounded-2xl bg-white border border-zinc-100/80 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(23,85,236,0.04)] p-4">
          ...
          <div className="space-y-2">
            {relatedTeachers.slice(0, 5).map((relatedTeacher) => {
              ...
              const image =
                relatedTeacher.image ?? `https://i.pravatar.cc/150?u=${relatedTeacher.id}`;
              return (
                <div
