...
export function StudentOverviewPanel({
  ...
  const quickActions: OverviewQuickAction[] = [
    ...(onView
      ? [{ icon: Eye, label: t('dashboard.common.actions.view'), onClick: () => onView(student!) }]
      : []),
    ...(onEdit
      ? [
          {
    ...
    ...(student?.phone
      ? [{ icon: Phone, label: student.phone, href: `tel:${student.phone}`, colSpan: 3 }]
      : []),
  ];
          ...
          student
            ? [
                ...
                },
                {
                  ...
                  value: new Date(student.enrollmentDate).toLocaleDateString('fr-FR'),
                },
              ]
            : []
        }
