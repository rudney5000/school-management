...
import { Star, Clock, MapPin, Pencil, Trash2, Radio, Copy, ExternalLink } from 'lucide-react';
...
const getCourseColor = (courseId: string) => {
  const index =
    courseId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % COURSE_COLORS.length;
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">{schedule.class.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <Avatar className="h-6 w-6">
                      <AvatarImage
                        ...
                        alt={teacherName}
                      />
                      <AvatarFallback className="bg-[#1755EC]/10 text-[#1755EC] text-[10px] font-semibold">
                        {getInitials(teacherName)}
                      </AvatarFallback>
                    </Avatar>
                    <span className="text-xs text-zinc-500">{getDayLabel(schedule.dayOfWeek)}</span>
                  </div>
