...
const OPTIONAL_PARENT_CATEGORIES: AttachmentCategory[] = ['parent_id', 'guardianship_proof'];
...
export function EnrollmentDocumentsUpload({ enrollmentId }: EnrollmentDocumentsUploadProps) {
  ...
  const getStatusBadge = (category: AttachmentCategory) => {
    ...
    if (!attachment) {
      return <Badge variant="outline">{t('dashboard.enrollment.documents.status.missing')}</Badge>;
    }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-semibold">{t('dashboard.enrollment.documents.title')}</h2>
