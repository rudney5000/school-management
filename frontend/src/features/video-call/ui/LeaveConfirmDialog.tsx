...
export function LeaveConfirmDialog({ isHost, onCancel, onConfirm }: LeaveConfirmDialogProps) {
      ...
      <div className="bg-neutral-900 rounded-xl p-6 w-full max-w-sm text-center space-y-4">
        ...
        <div className="flex gap-3 justify-center">
          <Button variant="ghost" onClick={onCancel}>
            {t('dashboard.chat.videoCall.leaveDialog.cancel')}
          </Button>
          <Button variant="destructive" onClick={onConfirm}>
            {isHost
              ? t('dashboard.chat.videoCall.leaveDialog.end')
              : t('videoCall.leaveDialog.leave')}
          </Button>
        </div>
      </div>
    </div>
