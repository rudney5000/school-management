...
workspace "School Management Architecture" "C4 architecture for the multi-school SaaS platform." {
    model {
        ...
        smsGateway = softwareSystem "android-sms-gateway (Cloud)" "External SMS gateway for OTP and notifications — one deviceId per sub-school, shared cloud account, school-owned SIM. See notifications.dsl." "External"
        ...
        schoolManagement = softwareSystem "School Management" "Multi-tenant, offline-first school management platform." {
            ...
            notificationsPkg = container "Notifications package" "packages/notifications — multi-identifier auth (email/phone/username) + SMS OTP, SMS dispatch pipeline. Internal to this repo. See notifications.dsl, ADR-008." "TypeScript, workspace package"
            postgres = container "PostgreSQL" "Tenant-aware transactional data and academic records." "PostgreSQL, Drizzle ORM"
            redis = container "Redis" "Cache, transient state, Socket.IO adapter data and Redis Streams for SMS dispatch." "Redis"
            ...
            auth = component "Authentication and authorization" "Multi-identifier login (email/phone/username), password and SMS OTP, roles and tenant access control." "TypeScript"
            ...
            communication = component "Communication" "Chat, events, notifications (via packages/notifications) and video call coordination." "TypeScript"
        ...
        api -> notificationsPkg "Auth and SMS notifications" "In-process import"
        notificationsPkg -> postgres "Via injected store interface (does not import api's db module directly, ADR-008)" "SQL, in-process interface"
        notificationsPkg -> redis "SMS Streams and OTP cache" "Redis"
