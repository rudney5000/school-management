...
export function registerChatHandlers(io: Server, socket: Socket, redis: Redis) {
  ...
  socket.on(
    'message:edit',
    async (data: { conversationId: string; messageId: string; input: EditMessageInput }) => {
      try {
  ...
  socket.on(
    'reaction:add',
    async (data: { conversationId: string; messageId: string; input: AddReactionInput }) => {
      try {
  ...
  socket.on(
    'reaction:remove',
    async (data: { conversationId: string; messageId: string; emoji: string }) => {
      try {
  ...
  socket.on(
    'message:forward',
    async (data: { conversationId: string; messageId: string; targetConversationId: string }) => {
      try {
