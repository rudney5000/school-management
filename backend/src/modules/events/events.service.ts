...
export class EventsService {
  async findAll(subSchoolId: string): Promise<EventRecord[]> {
    return db
      .select()
      .from(events)
      .where(and(eq(events.subSchoolId, subSchoolId)));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<EventRecord> {
    const [enrollment] = await db
      .select()
      .from(events)
      .where(and(eq(events.id, id), eq(events.subSchoolId, subSchoolId)));
  ...
  async update(id: string, subSchoolId: string, input: UpdateEventDto): Promise<EventRecord> {
  ...
  async remove(id: string, subSchoolId: string): Promise<void> {
    ...
    await db.delete(events).where(and(eq(events.id, id), eq(events.subSchoolId, subSchoolId)));
  }
}
