...
export class CoursesService {
  async findAll(subSchoolId: string): Promise<CourseRecord[]> {
    return db.select().from(courses).where(eq(courses.subSchoolId, subSchoolId));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<CourseRecord> {
    const [course] = await db
      .select()
      .from(courses)
      .where(and(eq(courses.id, id), eq(courses.subSchoolId, subSchoolId)));
  ...
  async update(id: string, subSchoolId: string, input: UpdateCourseDto): Promise<CourseRecord> {
  ...
  async remove(id: string, subSchoolId: string): Promise<void> {
    await this.findById(id, subSchoolId);
    await db.delete(courses).where(and(eq(courses.id, id), eq(courses.subSchoolId, subSchoolId)));
  }
  ...
  async removeCourseResource(id: string, courseId: string): Promise<void> {
    const [existing] = await db
      ...
      .where(and(eq(courseResources.id, id), eq(courseResources.courseId, courseId)));
  ...
  async resolveCoursesForParent(userId: string, subSchoolId: string) {
    ...
    return db
      .select()
      .from(courses)
      .where(and(inArray(courses.id, courseIds), eq(courses.subSchoolId, subSchoolId)));
  }
}
