...
export class SchedulesService {
  async findAll(subSchoolId: string): Promise<ScheduleRecord[]> {
    return db
      .select()
      .from(schedules)
      .where(and(eq(schedules.subSchoolId, subSchoolId)));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<ScheduleRecord> {
    const [schedule] = await db
      .select()
      .from(schedules)
      .where(and(eq(schedules.id, id), eq(schedules.subSchoolId, subSchoolId)));
  ...
  async update(id: string, subSchoolId: string, input: UpdateScheduleDto): Promise<ScheduleRecord> {
  ...
  async resolveSchedulesForParent(userId: string, subSchoolId: string): Promise<ScheduleRecord[]> {
    ...
    return db
      .select()
      .from(schedules)
      .where(and(inArray(schedules.classId, classIds), eq(schedules.subSchoolId, subSchoolId)));
  }
}
