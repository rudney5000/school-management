...
export class AcademicPeriodsService {
  async findAll(subSchoolId: string): Promise<AcademicPeriodRecord[]> {
    return db.select().from(academicPeriods).where(eq(academicPeriods.subSchoolId, subSchoolId));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<AcademicPeriodRecord> {
    const [period] = await db
      ...
      .where(and(eq(academicPeriods.id, id), eq(academicPeriods.subSchoolId, subSchoolId)));
  ...
  async create(
    input: CreateAcademicPeriodInput & { subSchoolId: string },
