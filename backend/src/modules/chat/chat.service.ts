...
export class ChatService {
  ...
  async findUserConversations(userId: string, subSchoolId: string) {
    ...
    return db.query.conversations.findMany({
      where: and(inArray(conversations.id, ids), eq(conversations.subSchoolId, subSchoolId)),
      with: {
        members: {
  ...
  async create(input: CreateConversationInput, createdBy: string, creatorRole: string) {
    if (creatorRole === 'parent') {
      ...
      if (input.memberIds.length !== 1) {
        throw new AppError('BAD_REQUEST', 'Un seul destinataire autorisé', 400);
      }
  ...
  async sendMessage(
    ...
    if (
      ...
      !['admin', 'director', 'super_admin'].includes(senderRole)
    ) {
      throw new AppError('FORBIDDEN', 'Ce canal est en lecture seule', 403);
    }
  ...
  async markAsRead(conversationId: string, userId: string, messageId: string) {
    await db.insert(messageReadReceipts).values({ messageId, userId }).onConflictDoNothing();
  ...
  async unarchiveMessage(messageId: string, userId: string) {
    await db
      .delete(messageArchives)
      .where(and(eq(messageArchives.messageId, messageId), eq(messageArchives.userId, userId)));
  }
  ...
  async forwardMessage(
    ...
    const targetConversation = await this.assertMemberAndGetConversation(
      targetConversationId,
      senderId,
    );

    if (
      targetConversation.type === 'announcement' &&
      !['admin', 'director', 'super_admin'].includes(senderRole)
    ) {
      throw new AppError('FORBIDDEN', 'Ce canal est en lecture seule', 403);
    }
  ...
  async findContactableStaff(userId: string, role: string, studentId?: string) {
    ...
    if (!parentRecord) throw new AppError('FORBIDDEN', 'Accès refusé', 403);
    ...
    const childStudents = await db
      ...
      .where(
        inArray(
          students.id,
          childrenLinks.map((c) => c.studentId),
        ),
      );
  ...
  private async findExistingDm(userA: string, userB: string) {
    ...
    return db.query.conversations
      .findFirst({
        where: and(sql`${conversations.id} = ANY(${ids})`, eq(conversations.type, 'dm')),
        with: { members: true },
      })
      .then((conv) => {
        if (!conv) return null;
  ...
  private async withAttachmentUrls<T extends { attachments?: { key: string }[] }>(
    message: T,
  ): Promise<T> {
  ...
  private async assertParentCanMessageStaff(parentUserId: string, targetUserId: string) {
    ...
    if (!target) throw new AppError('FORBIDDEN', 'Destinataire invalide', 403);
    ...
    if (['admin', 'director', 'super_admin'].includes(target.role)) {
      ...
      if (!parentUser) throw new AppError('FORBIDDEN', 'Accès refusé', 403);
    ...
    throw new AppError('FORBIDDEN', 'Destinataire invalide', 403);
  }
  ...
  async resolveParentId(userId: string): Promise<string> {
    ...
    if (!user?.parentId) throw new AppError('FORBIDDEN', 'Accès refusé', 403);
  ...
  private async assertTeacherLinkedToParent(teacherUserId: string, parentUserId: string) {
    ...
    if (!teacherUser?.teacherId) throw new AppError('FORBIDDEN', 'Accès refusé', 403);
