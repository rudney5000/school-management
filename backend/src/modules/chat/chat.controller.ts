...
export class ChatController {
  ...
  editMessage = asyncHandler(async (req: Request, res: Response) => {
    ...
    const data = await this.service.editMessage(req.params.messageId, req.user!.id, input);
    respond(res, data);
  });
  ...
  removeReaction = asyncHandler(async (req: Request, res: Response) => {
    await this.service.removeReaction(req.params.messageId, req.user!.id, req.params.emoji);
    res.status(204).send();
  });
  ...
  getThreadReplies = asyncHandler(async (req: Request, res: Response) => {
    const data = await this.service.findThreadReplies(req.params.messageId, req.user!.id);
    respond(res, data);
  });
