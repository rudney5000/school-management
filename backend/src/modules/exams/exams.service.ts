...
import { mapExamTypeToGradeType } from '@/modules/exams/exam-type-to-grade-type.mapper';
import { GradesService } from '@/modules/grades/grades.service';
import { courses, enrollments, parentStudents, users } from '@/db/schema';
...
export class ExamsService {
  ...
  async findById(id: string, subSchoolId: string): Promise<ExamRecord> {
    const [exam] = await db
      .select()
      .from(exams)
      .where(and(eq(exams.id, id), eq(exams.subSchoolId, subSchoolId)));
  ...
  async create(
    input: CreateExamInput & { subSchoolId: string; createdBy: string },
  ): Promise<ExamRecord> {
    const [exam] = await db
      .insert(exams)
      .values({
  ...
  async delete(id: string, subSchoolId: string): Promise<void> {
    ...
    await db.delete(exams).where(and(eq(exams.id, id), eq(exams.subSchoolId, subSchoolId)));
  }
  ...
  async resolveExamsForParent(userId: string, subSchoolId: string): Promise<ExamRecord[]> {
    ...
    return db
      .select()
      .from(exams)
      .where(and(inArray(exams.classId, classIds), eq(exams.subSchoolId, subSchoolId)));
  }
}
...
export class ExamResultsService {
  constructor(private readonly gradesService: GradesService = new GradesService()) {}

  async findByExam(examId: string, subSchoolId: string): Promise<ExamResultRecord[]> {
    const [exam] = await db
      .select()
      .from(exams)
      .where(and(eq(exams.id, examId), eq(exams.subSchoolId, subSchoolId)));

    if (!exam) {
      throw new AppError('NOT_FOUND', 'Examen introuvable', 404);
    }

    return db.select().from(examResults).where(eq(examResults.examId, examId));
  }
  ...
  async findByStudent(studentId: string, subSchoolId: string): Promise<ExamResultRecord[]> {
    return db
      ...
      .where(and(eq(examResults.studentId, studentId), eq(exams.subSchoolId, subSchoolId)));
  }
  ...
  async bulkUpsert(
    ...
    const [exam] = await db
      .select()
      .from(exams)
      .where(and(eq(exams.id, input.examId), eq(exams.subSchoolId, subSchoolId)));
  ...
  private async syncExamCompletionStatus(
    ...
    const [{ gradedCount }] = await db
      ...
      .from(examResults)
      .where(and(eq(examResults.examId, examId), sql`${examResults.score} is not null`));
