...
import { videoCallParticipants, videoCallSessions } from '@/db/schema/videoCallSessions';
...
export class VideoCallsService {
  ...
  async findById(sessionId: string, subSchoolId: string): Promise<VideoCallSessionRecord> {
    const [session] = await db
      ...
      .from(videoCallSessions)
      .where(
        and(eq(videoCallSessions.id, sessionId), eq(videoCallSessions.subSchoolId, subSchoolId)),
      );
  ...
  async end(sessionId: string, subSchoolId: string, requesterId: string): Promise<void> {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .set({ leftAt: new Date() })
        .where(
          and(eq(videoCallParticipants.sessionId, sessionId), isNull(videoCallParticipants.leftAt)),
        );
    });
  }
}
