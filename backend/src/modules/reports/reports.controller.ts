...
export class ReportsController {
    ...
    create = asyncHandler(async (req: Request, res: Response): Promise<void> => {
        if (!ALLOWED_REPORTER_ROLES.includes(req.user!.role as (typeof ALLOWED_REPORTER_ROLES)[number])) {
            throw new AppError(
                'FORBIDDEN',
                'Ce rôle ne peut pas créer de signalement',
                403
            );
        }
        ...
        if (!subSchoolId) {
            throw new AppError(
                'BAD_REQUEST',
                'Impossible de déterminer la sous-école',
                400
            );
        }
