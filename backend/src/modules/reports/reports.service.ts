...
import { db } from '@/db';
import { reports, reportStatusHistory } from '@/db/schema/reports';
...
import { emitToReportRoom, emitToUserRoom } from '@/socket/middleware/reports.socket';
import { parentStudents, users } from '@/db/schema';
...
export class ReportsService {
  async create(input: CreateReportDto, ctx: ReportActorContext): Promise<ReportRecord> {
    if (input.relatedStudentId && ctx.role === 'parent') {
      ...
      if (!isOwnChild) {
        throw new AppError('FORBIDDEN', 'Élève non lié à ce compte parent', 403);
      }
    }
  ...
  async findById(id: string, subSchoolId: string): Promise<ReportRecord> {
    const [report] = await db
      .select()
      .from(reports)
      .where(and(eq(reports.id, id), eq(reports.subSchoolId, subSchoolId)));
  ...
  async findByTrackingToken(token: string) {
    const [report] = await db.select().from(reports).where(eq(reports.trackingToken, token));
  ...
  async updateStatus(
    ...
    const [updated] = await db
      ...
      .set({
        ...
        updatedAt: new Date(),
        ...(input.status === 'resolved' ? { resolvedAt: new Date(), resolvedById: actorId } : {}),
      })
  ...
  async assign(id: string, input: AssignReportDto, subSchoolId: string): Promise<ReportRecord> {
