...
import { liveSessions, liveSessionViewers } from '@/db/schema/liveSessions';
...
export class LiveSessionsService {
  async findAll(subSchoolId: string): Promise<LiveSessionRecord[]> {
    return db.select().from(liveSessions).where(eq(liveSessions.subSchoolId, subSchoolId));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<LiveSessionRecord> {
    const [session] = await db
      .select()
      .from(liveSessions)
      .where(and(eq(liveSessions.id, id), eq(liveSessions.subSchoolId, subSchoolId)));
  ...
  async end(id: string, subSchoolId: string, requesterId: string): Promise<LiveSessionRecord> {
    ...
    await db
      ...
      .set({ leftAt: new Date() })
      .where(and(eq(liveSessionViewers.sessionId, id), isNull(liveSessionViewers.leftAt)));
