...
import { db } from '@/db';
import { attachments, teachers, teacherSchools } from '@/db/schema';
...
export class TeachersService {
  async findAll(subSchoolId: string): Promise<TeacherWithAssignment[]> {
    return db
      ...
      .where(and(eq(teacherSchools.subSchoolId, subSchoolId), eq(teacherSchools.isActive, true)));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<TeacherWithAssignment> {
    const [row] = await db
      ...
      .from(teacherSchools)
      .innerJoin(teachers, eq(teacherSchools.teacherId, teachers.id))
      .where(and(eq(teachers.id, id), eq(teacherSchools.subSchoolId, subSchoolId)));
  ...
  async updateAssignment(
    ...
    await db
      ...
      .where(and(eq(teacherSchools.teacherId, id), eq(teacherSchools.subSchoolId, subSchoolId)));
  ...
  async getDossierStatus(teacherId: string, subSchoolId: string) {
    ...
    const docs = await db
      ...
      .from(attachments)
      .where(
        and(eq(attachments.attachableType, 'teacher'), eq(attachments.attachableId, teacherId)),
      );
