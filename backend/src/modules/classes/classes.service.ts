...
import { db } from '@/db';
import { classCourses, classes } from '@/db/schema';
import { AppError } from '@/shared/errors/app-error';
import type { CreateClassCourseDto, CreateClassDto, UpdateClassDto } from './classes.schema';
...
export class ClassesService {
  async findAll(subSchoolId: string): Promise<ClassRecord[]> {
    return db.select().from(classes).where(eq(classes.subSchoolId, subSchoolId));
  }
  ...
  async findById(id: string, subSchoolId: string): Promise<ClassRecord> {
    const [classRecord] = await db
      .select()
      .from(classes)
      .where(and(eq(classes.id, id), eq(classes.subSchoolId, subSchoolId)));
  ...
  async update(id: string, subSchoolId: string, input: UpdateClassDto): Promise<ClassRecord> {
  ...
  async remove(id: string, subSchoolId: string): Promise<void> {
    await this.findById(id, subSchoolId);
    await db.delete(classes).where(and(eq(classes.id, id), eq(classes.subSchoolId, subSchoolId)));
  }
