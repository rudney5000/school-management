...
import { examStatusEnum, examTypeEnum } from '@/db/schema/enums';
...
import { liveSessionColumns } from '@/db/schema/liveSessionColumns';
...
export const exams = pgTable('exams', {
  ...
  retakeOfExamId: uuid('retake_of_exam_id').references((): AnyPgColumn => exams.id, {
    onDelete: 'set null',
  }),
  ...liveSessionColumns,
...
export const examResults = pgTable(
  ...
  (t) => ({
    uniqueResult: uniqueIndex('exam_results_exam_student_unique').on(t.examId, t.studentId),
  }),
);
