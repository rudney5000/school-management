...
import { conversationTypeEnum, memberRoleEnum, messageTypeEnum } from '@/db/schema/enums';
...
export const conversationMembers = pgTable(
  ...
  (table) => ({
    uniq: uniqueIndex('uniq_conversation_member').on(table.conversationId, table.userId),
  }),
);
...
export const messageReactions = pgTable(
  ...
  (table) => ({
    uniq: uniqueIndex('uniq_message_reaction').on(table.messageId, table.userId, table.emoji),
  }),
);
...
export const messageReadReceipts = pgTable(
  ...
  (table) => ({
    uniq: uniqueIndex('uniq_message_read_receipt').on(table.messageId, table.userId),
  }),
);
...
export const messageStars = pgTable(
  ...
  (table) => ({
    uniq: uniqueIndex('uniq_message_star').on(table.messageId, table.userId),
  }),
);

export const messageArchives = pgTable(
  'message_archives',
  {
    messageId: uuid('message_id')
      .notNull()
  ...
  (table) => ({
    uniq: uniqueIndex('uniq_message_archive').on(table.messageId, table.userId),
  }),
);
