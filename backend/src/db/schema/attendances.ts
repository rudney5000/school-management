import { pgTable, uuid, varchar, date, timestamp, uniqueIndex, text } from 'drizzle-orm/pg-core';
import { relations, sql } from 'drizzle-orm';
import { students } from './students';
import { teachers } from './teacher';
import { attendanceStatusEnum, attendanceTargetEnum } from './enums';
...
export const teacherAttendances = pgTable(
  ...
  (table) => ({
    idx_teacher_attendance_unique: uniqueIndex('idx_teacher_attendance_unique').on(
      table.teacherId,
      table.date,
    ),
  }),
...
export const studentAttendancesRelations = relations(studentAttendances, ({ one }) => ({
  ...
  subSchool: one(subSchools, {
    fields: [studentAttendances.subSchoolId],
...
export const teacherAttendancesRelations = relations(teacherAttendances, ({ one }) => ({
  ...
  subSchool: one(subSchools, {
    fields: [teacherAttendances.subSchoolId],
    references: [subSchools.id],
  }),
}));
